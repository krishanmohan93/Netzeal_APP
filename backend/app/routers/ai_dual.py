"""AI Router - NVIDIA Integrate backed endpoints."""
...
from ..core.feature_gates import require_experimental_ai_enabled

logger = logging.getLogger(__name__)

router = APIRouter(
    prefix="/ai-dual",
    tags=["AI Dual Provider"],
    dependencies=[Depends(require_experimental_ai_enabled)],
)
...
class ChatRequest(BaseModel):
    ...
    mode: Literal["free", "deep"] = Field(default="free", description="Compatibility mode: free or deep")
...
class CaptionRequest(BaseModel):
    ...
    premium: bool = Field(default=False, description="Compatibility flag for premium-like behavior")
...
async def chat_with_ai(request: ChatRequest):
    """
    Chat with AI via NVIDIA Integrate.

    - **mode="free"** and **mode="deep"** are kept for backward compatibility.
    """
    try:
...
async def extract_hashtags(request: TagsRequest):
    """
    Extract relevant hashtags from caption or post content.
        ...
        "providers": [
            {
                ...
                "features": ["OpenAI-compatible API", "High-quality reasoning", "Unified provider"],
